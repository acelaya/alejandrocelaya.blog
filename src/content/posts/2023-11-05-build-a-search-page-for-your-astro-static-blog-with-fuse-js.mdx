...
For that, you will need an [island](https://docs.astro.build/en/concepts/islands/), a component with logic that runs in the browser. We'll be using react to write that component, and it will receive a list of posts and use Fuse with them.
...
In this blog I use Astro's [content collections](https://docs.astro.build/en/guides/content-collections/) to handle the posts. That allows me to take advantage of the helpers exposed by `astro:content`.
...
If you write your posts as regular pages, it's slightly harder to get the bodies of your posts, but you can use [`Astro.glob()`](https://docs.astro.build/en/reference/api-reference/#astroglob) to list your posts and metadata, and then manually render each post to get its body.
...
As you can see, the second option is a bit less convenient, so I would recommend using content collections if possible.

### Embedded search vs endpoint

The next thing you need to decide is how the `Search` component will get the list of posts.

You need to take into consideration that you are exposing the whole body of every post so that fuse can search on it. That means the list can be relatively big depending on how many posts you have.

In the examples above we have embedded the list of posts in the search page itself. When you build it you'll see all posts are part of the generated `search.html` file, which can make it relatively big.
...
With that, you can define an individual caching strategy for this JSON file itself, load the search page faster and display a loading state, etc.

First, we would need to create the endpoint that would serve the list of posts.
...
> Take into consideration this is simplified. You should probably handle potential errors when loading the posts, abort the request when the component unmounts, etc.
...
However, if you decide to pre-generate the index, you should not embed it in the search page, and instead expose it via an endpoint to avoid a very big `search.html` file.
