...
[FrankenPHP](https://frankenphp.dev/) is one of the latest additions to the PHP application server ecosystem. It was created by [Kévin Dunglas](https://github.com/dunglas), and recently started being officially supported by [The PHP Foundation](https://thephp.foundation/blog/2025/05/15/frankenphp/).
...
As you can see, there's a `$handler` callback, which is invoked for every HTTP request. In this example it simple echoes the result of `$myApp->handle(...)`, being `$myApp` and instance of a hypothetical application.
...
There are other alternative servers, like [RoadRunner](https://roadrunner.dev/), which is in fact more performant than FrankenPHP (in my experience) and has other features on top, like the ability to run background jobs.
