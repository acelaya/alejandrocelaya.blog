...
* [shlink-js-sdk](https://github.com/shlinkio/shlink-js-sdk): It wraps the logic to consume a Shlink API from a Javascript app. It exposes type contracts for shlink-web-component, and implementations for consumers running in the browser (shlink-web-client) or the backend (Shlink Dashboard).
...
* It's much easier to test via snapshots, as it renders charts via `SVG` elements, which gets reflected in the DOM tree. Chart.js renders via `Canvas` elements, so you need to somehow capture events in order to check the chart is rendered as expected.
...
* The official Docker image no longer runs as root, which improves security, and is frequently a requirement for many companies.
  ...
  As a side effect, this means it can no longer expose port 80, but 8080.
...
* **Add E2E tests with [Playwright](https://playwright.dev/)**: This is another task that I have been delaying for a long time, but shlink-web-client will reach v5.0.0 with E2E tests.
* **Improve color contrast**: Many accessibility improvements have been introduced, but there are still others left.

  The most important ones are around color contrast, and specifically, around the fact that Shlink's brand color (`#4696e5`) doesn't have enough contrast with white backgrounds.
