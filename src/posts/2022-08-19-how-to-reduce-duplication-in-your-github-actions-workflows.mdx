...
jobs:
  static-checks:
    ...
    strategy:
      matrix:
        command: ['soding-styles', 'static-analysis']
    steps:
      - name: Checkout code
        uses: actions/checkout@v3
      - name: Setup cache environment
...
The next more obvious thing is that there are a couple of steps that appear on each one of the workflows to set up the environment.
...
```yaml
name: CI setup
description: 'Sets up the environment to jobs during CI workflow'
...
Also, as a side effect, we got rid of defining the PHP extensions as an env var, since we now pass them as an arg of the action only in one place.
...
They are similar to composite actions, with the difference that they do not wrap only a couple of steps, but they can have even multiple jobs that we programmatically invoke.
...
There are a couple of things to clarify from the example above:
